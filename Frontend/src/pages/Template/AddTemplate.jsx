...
import { TeacherDetailFields } from '../../config/FormFieldConfig/UserFieldConfig/teacherDetailFields';
...
const AddTemplate = () => {
  ...
  // On component mount or route change, initialize fields
  useEffect(() => {
    const templateJson = getTemplateJson(userType);
    const initialFieldData = templateJson.map(field => ({
      name: field.label,
      selected: false,
  ...
  const postTemplate = async fields => {
    ...
    try {
      await dispatch(addTemplateThunk({ payload }));
    } catch (error) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      await postTemplate(selectedFields); // 2. Then call the API to save config
    } catch (error) {
      ...
      <Grid item xs={12}>
        <Paper elevation={2} sx={{ px: 3, py: 2 }}>
          <Typography variant='h4' gutterBottom>
            Create {userType.charAt(0).toUpperCase() + userType.slice(1)} Template
          </Typography>
        </Paper>
      </Grid>
